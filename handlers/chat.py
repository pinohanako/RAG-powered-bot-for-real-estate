...
async def sticker(message: Message):
    ...
    if data_extracted["used_sticker_phrases"] is None:
        ...
        used_phrases_stickers = f"{data_extracted['used_sticker_phrases']}"
    ...
    if len(used_phrases_stickers) < len(funny_phrases):
        ...
        random_phrase = random.choice(unused_phrases)
        used_phrases_stickers.append(random_phrase)
        selected_stickers = {"stickers": used_phrases_stickers}
        used_phrases_stickers_value = f"{selected_stickers['stickers']}"

        await asyncio.sleep(5)
        cursor.execute("UPDATE session_store SET used_sticker_phrases = %s WHERE user_id = %s", (used_phrases_stickers_value, chat_id))
        conn.commit()
        await message.answer(random_phrase)
    else:
        await asyncio.sleep(3)
